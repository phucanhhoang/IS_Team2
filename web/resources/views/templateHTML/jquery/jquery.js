...
$(function () {
    ...
    //Slide-detail
    var img_src = $('.slide-detail .list img').attr("src");
    $('.slide-detail .large').css("background", "url(" + img_src + ")");

    $('.slide-detail .list img').click(function () {
        var img_src = $(this).attr("src");
        $('.slide-detail .small').attr("src", img_src);
        $('.slide-detail .large').css("background", "url(" + img_src + ")");
    });
    ...
    var init_w = 0;
    var init_h = 0;
    $('.slide-detail .detail').mousemove(function (e) {
        if (!init_w && !init_h) {
            var image = new Image();
            image.src = $('.slide-detail .small').attr("src");
            init_w = image.width;
            init_h = image.height;
        }
        else {
            ...
            if (mx < $(this).width() - 1 && my < $(this).height() - 1 && mx > 1 && my > 1) {
                $('.slide-detail .large').fadeIn(100);
                console.log("mx:" + mx + " my:" + my + " w:" + $(this).width() + " h:" + $(this).height());
            }
            else {
                $('.slide-detail .large').fadeOut(100);

            }
            if ($('.slide-detail .large').is(":visible")) {
                ...
                $('.slide-detail .large').css("background-position", bg_position);
                $('.slide-detail .large').css("width", $('.slide-detail .detail').width() + "px");
                $('.slide-detail .large').css("height", $('.slide-detail .detail').height() + "px");
            }
